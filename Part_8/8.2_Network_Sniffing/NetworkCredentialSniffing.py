...
"""
Extract credentials from pcaps containing insecure protocols: FTP, SMTP and Telnet
"""
...
def ExtractSMTP(packet):
    ...
    try:
        # Detect authentication packets based on presence of b64 data
        ...
        if re.search(emailregex, decoded):
            ...
            # Add the source IP address and port of this packet to the list of sources we are waiting for a password from
            # This allows us to associate usernames and passwords even if multiple hosts are authenticating to the SMTP server
...
def ExtractTelnet(packet):
    try:
        # Check for UTF-8 encoded packets (authentication packets)
        payload = packet[Raw].load.decode("utf-8").rstrip()
    except:
        return
    connData = [packet[IP].src, packet[TCP].sport]  # Assume server is source
    if payload[:5] == "login":
        # Add packet to the list of login prompt packets we are waiting for a response to
        awaitingLogin.append(connData)
        return
    elif payload[:8] == "Password":
        # Add packet to the list of password prompt packets we are waiting for a reponse to
...
for packet in packets:
    ...
    if packet.haslayer(TCP) and packet.haslayer(Raw):
        # Determine credentials to extract based on packet destination port
